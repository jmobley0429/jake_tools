...
class PIE_MT_AddModifier(Menu):
    ...
    def draw(self, context):
        ...
        spl = box.split()
        op = spl.operator(
            "object.custom_mirror_modifier", text="X -", icon="MOD_MIRROR"
        )
        ...
        spl = box.split()
        op = spl.operator(
            "object.custom_mirror_modifier", text="Y -", icon="MOD_MIRROR"
        )
        ...
        spl = box.split()
        op = spl.operator(
            "object.custom_mirror_modifier", text="Z -", icon="MOD_MIRROR"
        )
        ...
        box.operator(
            "object.custom_bevel_subsurf_modifier",
            text="Bevel Subsurf",
            icon="MOD_SUBSURF",
        )
        op = box.operator(
            "object.auto_crease_subdivide", text="Auto Crease Subdiv", icon="SNAP_EDGE"
        )

        # Bottom -- Deform
        box = pie.split().column()
        box.operator(
            "object.custom_simple_deform", text="Bend", icon="MOD_SIMPLEDEFORM"
        )
        box.operator("object.custom_displace", text="Displace", icon="MOD_DISPLACE")
        box.operator(
            "object.custom_shrinkwrap", text="Shrinkwrap", icon="MOD_SHRINKWRAP"
        )
        box.operator(
            "object.custom_lattice", text="Lattice", icon="OUTLINER_DATA_LATTICE"
        )
        # Top -- Mesh \ Edges
        box = pie.split().column()
        ...
        # BL --
        op = pie.operator(
            "object.multiple_modifiers_add", text="Geometry Nodes", icon="NODETREE"
        )
        op.mod_type = "NODES"
        # BL --
