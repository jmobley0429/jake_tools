...
class PIE_MT_AddOtherObjects(Menu):
    ...
    def draw(self, context):
        ...
        pie.operator(
            "image.import_as_mesh_planes", text="Image as Plane", icon="FILE_IMAGE"
        )
        # Text
        ...
        spl = box.split()
        op = spl.operator("object.grease_pencil_add", text="Blank", icon="EMPTY_AXIS")
        op.type = "EMPTY"
        op = spl.operator(
            "object.grease_pencil_add", text="Stroke", icon="OUTLINER_OB_GREASEPENCIL"
        )
        op.type = "STROKE"
        spl = box.split()
        op = spl.operator(
            "object.grease_pencil_add", text="Line Art Object", icon="OBJECT_DATA"
        )
        op.type = "LINEART_OBJECT"
        spl = box.split()
        op = spl.operator(
            "object.grease_pencil_add",
            text="Line Art Collection",
            icon="OUTLINER_COLLECTION",
        )
        op.type = "LINEART_COLLECTION"
