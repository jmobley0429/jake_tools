...
class PIE_MT_mesh_flatten(Menu):
    ...
    def draw(self, context):
        ...
        for axis in "XYZ":
            row = box.row()
            op = row.operator(
                "transform.flatten_to_selection_bounding_box", text=f"    -{axis}"
            )
            op.axis = axis
            op.side = "NEGATIVE"
            op = row.operator(
        ...
        # 2 - BOTTOM
        pie.operator(
            "transform.flatten_to_origin", text="Origin Y", icon="OBJECT_ORIGIN"
        ).axis = "Y"
        # 8 - TOP
        pie.operator(
            "transform.flatten_to_center", text="Center Y", icon="ORIENTATION_GLOBAL"
        ).axis = "Y"
        # 7 - TOP - LEFT
        pie.operator(
            "transform.flatten_to_center", text="Center X", icon="ORIENTATION_GLOBAL"
        ).axis = "X"
        # 9 - TOP - RIGHT
        pie.operator(
            "transform.flatten_to_center", text="Center Z", icon="ORIENTATION_GLOBAL"
        ).axis = "Z"
        # 1 - BOTTOM - LEFT
        pie.operator(
            "transform.flatten_to_origin", text="Origin X", icon="OBJECT_ORIGIN"
        ).axis = "X"
        # 3 - BOTTOM - RIGHT
        pie.operator(
            "transform.flatten_to_origin", text="Origin Z", icon="OBJECT_ORIGIN"
        ).axis = "Z"
...
kms = [
    {
        ...
        "space_type": "VIEW_3D",
        "region_type": "WINDOW",
        "keywords": {"name": PIE_MT_mesh_flatten.bl_idname},
    }
]
