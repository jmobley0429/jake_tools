...
class PIE_MT_init_face_sets(Menu):
    ...
    def draw(self, context):
        ...
        pie = layout.menu_pie()
        op = pie.operator(
            "sculpt.face_sets_init", text="Loose Parts", icon="OUTLINER_DATA_POINTCLOUD"
        )
        op.mode = "LOOSE_PARTS"
        op = pie.operator(
            "sculpt.face_sets_init", text="Face Set Boundaries", icon="PIVOT_BOUNDBOX"
        )
        op.mode = "FACE_SET_BOUNDARIES"
        op = pie.operator("sculpt.face_sets_init", text="Materials", icon="MATERIAL")
        op.mode = "MATERIALS"
        op = pie.operator(
            "sculpt.face_sets_init", text="Normals", icon="NORMALS_VERTEX_FACE"
        )
        op.mode = "NORMALS"
        ...
        op.mode = "CREASES"
        op = pie.operator(
            "sculpt.face_sets_init", text="Edge Bevel Weight", icon="MOD_BEVEL"
        )
        op.mode = "BEVEL_WEIGHT"
        op = pie.operator(
            "sculpt.face_sets_init", text="Sharp Edges", icon="SHARPCURVE"
        )
        op.mode = "SHARP_EDGES"
